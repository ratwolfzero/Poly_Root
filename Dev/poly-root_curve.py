...
def plot_roots(roots_mp, equation, coeffs):
    ...
    button = Button(button_ax, 'Poly Curve', color='lightgoldenrodyellow', hovercolor='gold')
...
def plot_polynomial_curve(coeffs, roots_mp, equation):
    ...
    tol = 1e-7
    real_roots_to_mark = [r for i, r in enumerate(real_parts) if abs(float(mp.im(roots_mp[i]))) < tol]
    if real_roots_to_mark:
        plt.plot(real_roots_to_mark, [0]*len(real_roots_to_mark), "ro", markersize=8, label="Real Roots")
